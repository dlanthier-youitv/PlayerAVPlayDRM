...
function VideoPlayer(config) {
    ...
    var DownloadCompletedCallback = {
    		oncompleted: function(id, downloadpath) {
        	  		log("****wow Debug - Finished downloading subtitles path: " + downloadpath);
        	  		log("****wow Debug - Finished downloading subtitles this: " + this.subtitleFilePath);
        	  		subtitleFilePath = downloadpath;
        }.bind(this)
    };
        ...
        play: function (url) {
            ...
            var listener = {
                ...
                ondrmevent: function (drmEvent, drmData) {
                    ...
                    //log("DANHACK DRM Event" + drmEvent + ", data: " + JSON.stringify(drmData));
                    /* Playready Challenge alternative scenario start */
                    if (drmData.name === "Challenge") {
                       // log("DANHACK Challenge was received.");

                        var drmParam = {
                            ResponseMessage: drmData.message
                        };
                        log('*** Before setDrm InstallLicense: ' + JSON.stringify(drmParam));
                ...
                onerror: function (eventType) {
	        		...
	        		onsubtitlechange: function(duration, text, type, attriCount, attributes) {
	        			log('danhack - subtitle change: ' + text);
            ...
            /* Setting DRMs */
            switch (chosenDrm) {
            		case config.drms.TTV_DASH_1:
            		case config.drms.TTV_DASH_2:
            		case config.drms.TTV_SMOOTH:
            		case config.drms.MS_SMOOTH_PLAYREADY:
            		case config.drms.MS_DASH_DRM:
                case config.drms.PLAYREADY:
                    this.setPlayready();
                    break;
                case config.drms.TTV_DASH_CHALLENGE:
                case config.drms.PLAYREADY_GET_CHALLENGE:
                    this.setPlayreadyChallenge();
                    break;
                case config.drms.WIDEVINE:
                case config.drms.TTV_DASH_WV:
                    this.setWidevine();
                    break;
                default:
                    log('no DRM');
            ...
            if (chosenDrm !== config.drms.PLAYREADY_GET_CHALLENGE) {
            		log('Danhack - before preparing (sync)');
            		
                    log('***** - before preparing (async)');
                    this.downloadSubtitles();
                    webapis.avplay.prepareAsync(() => {
                        log('**** 2 PrepareSuccessCallback');
                        webapis.avplay.play();
                        log('**** 2 PrepareSuccessCallback - after play');
...
//                log('Danhack - after preparing (sync)');
//                webapis.avplay.play();
            }
        },
        /**
        ...
        setPlayready: function () {
    		log('DANHACK - setDrm Playready. Player State: ' + webapis.avplay.getState());
        var drmParam = {
        ...
        if (chosenDrm.token !== '' && chosenDrm.token !== undefined) {
        		drmParam.HttpHeader = 'Authorization: ' + chosenDrm.token;
        }
        ...
        setPlayreadyChallenge: function () {
            ...
            var PrepareSuccessCallback = function () {
                ...
                webapis.avplay.play();
                log('**** PrepareSuccessCallback - after play');
                enableHardcodedSubtitles();
            };
        ...
        downloadSubtitles: function() {
			log('*** DANHACK - DOWNLOADING SUBTITLES 4');

			try {
//                webapis.avplay.setSilentSubtitle(true);
        			var url = 'https://static.tou.tv/medias/webvtt/1995-02-20_19_30_00_LAPETITEVIE_0027.vtt';
        			var request = new tizen.DownloadRequest(url, 'wgt-private-tmp', 'subtitles');        			
                tizen.download.start(request, DownloadCompletedCallback);
			} catch (e) {
    				log(e);
			}
        },
        ...
        enableSubtitles: function() {
			log('*** DANHACK - ENABLING SUBTITLES');

        		try {
        			if (subtitleFilePath !== "" && subtitleFilePath !== undefined) {
        				log('*** DANHACK - Before setExternalSubtitlePath');

    	                webapis.avplay.setExternalSubtitlePath(subtitleFilePath);
    	                webapis.avplay.setSilentSubtitle(false);
        			}
        			else {
        				log('*** DANHACK - Do not have a subtitleFilePath SUBTITLES');
        			}
        		} catch (e) {
        			log(e);
        		}
